...
'''net = network.Network(
    [network.FullyConnectedLayer(784, 100),
     network.FullyConnectedLayer(100, 10)]
)'''
training_data, validation_data, test_data = mnist_loader.load_data_wrapper()
training_data = list(training_data)
test_data = list(test_data)
print("finishing mnist_digits_recognition")
...
net = network.Network(
    [network.ConvPoolLayer(image_shape=(1, 28, 28),
                           filter_shape=(20, 1, 5, 5),
                           poolsize=(2, 2)),
     network.FullyConnectedLayer(n_in=(20 * 12 * 12), n_out=50),
     network.FullyConnectedLayer(50, 10)]
)
net.SGD(training_data[:100], 10, 10, 10.5,
        evaluation_data=test_data[0:10], lmbda=1,
        monitor_evaluation_accuracy=True, monitor_evaluation_cost=False,
        monitor_training_accuracy=True, monitor_training_cost=False)
